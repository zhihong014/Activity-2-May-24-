...
#Open a new csv file
def open_new():
    with open('MALAYSIA TAX RELIEF.csv', mode='w') as open_file:
        header = ['User_ID', 'User_Full_IC', 'User Password', 'Salary per year', 'Total tax relief', 'Salary after tax reief', 'Taxrate', 'Taxpayable']
        writer = csv.DictWriter(open_file, fieldnames=header)
        writer.writeheader()
...
def login():
    ...
    with open('MALAYSIA TAX RELIEF.csv', mode='r') as read:
...
def registration():
    ...
    user_ic = int(input('Password (IC):'))
    count = len(str(user_ic))

    if count == 12:
        print('Your registration is succussfully! Thank You!')
        print('Your password is: ', user_ic % 10000)

        with open('MALAYSIA TAX RELIEF.csv', mode='a', newline='') as register:
            writer = csv.writer(register, delimiter=',')
            user_id = user_id_r
            password = user_ic % 10000
            writer.writerow([user_id, user_ic, password])
...
def salary_calculation():
    ...
    if salary > 0:
        ...
        def submit():
            ...
            with open('MALAYSIA TAX RELIEF.csv', mode='a', newline='') as store:
                header = ['Salary per year', 'Total tax relief', 'Salary after tax reief', 'Taxrate', 'Taxpayable']
                writer = csv.DictWriter(store, fieldnames=header, delimiter=',')
